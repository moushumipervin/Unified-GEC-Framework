...
###############################################################################
# A2: Find fixed PS1 intercept producing desired marginal treatment rate
#
# PS1:
#
# pi(X) = expit{ -(a + strength * linear_predictor(X)) }
#
###############################################################################

find_ps1_intercept <- function(
    target_rate,
    ps_strength = 2,
    M = 1000000,
    seed = 92345
...
generate_ate_data <- function(
  ...
  if (ps_model == 1) {
    ...
    # Original value when doing the ordinary simulation
    if (is.null(ps_intercept)) {
      ps_intercept <- 0.25
    }
  ...
  #############################################################################
  # Estimated PS used by estimators
  #
  # Keep same working PS1 model.
  #############################################################################
...
################################################################################
# A2 SEVERE-OVERLAP / HEAVY-TAIL STRESS TEST
#
# Methods:
#   IPW
#   AIPW-GAM
...
#   MAXW1 / MAXW0
#   success indicator
#   realized treatment rate
#   true propensity diagnostics
################################################################################

run_one_replication_A2_stress <- function(
    n = 1000,
    p = 4,
    K = 4,
    rep_id = 1,
  ...
  ##############################################################################
  # 1. Generate OR2PS1 data under A2 stress design
  ##############################################################################
  ...
  ##############################################################################
  # 3. Cross-fitted GAM outcome predictions
  #
  # Same predictions used for:
  #   AIPW
  #   SL / EL / ET / HD / CE
  #
  ...
  if (!is.null(fold_T1_gam) &&
    ...
    if (!is.null(aipw_inf)) {
      ...
      benchmark_output["AIPW_GAM_MAXW1"] <-
        aipw_inf$max_weight1
      
      benchmark_output["AIPW_GAM_MAXW0"] <-
        aipw_inf$max_weight0
      
      benchmark_output["AIPW_GAM_SUCCESS"] <-
        as.integer(
          isTRUE(aipw_inf$success)
        )
      
    } else {
      
      estimates["AIPW_GAM"] <- NA_real_
      
      benchmark_output["AIPW_GAM_SE_ANA"] <- NA_real_
      benchmark_output["AIPW_GAM_COV_ANA"] <- NA_real_
      
      benchmark_output["AIPW_GAM_ESS1"] <- NA_real_
      benchmark_output["AIPW_GAM_ESS0"] <- NA_real_
      
      benchmark_output["AIPW_GAM_MAXW1"] <- NA_real_
      benchmark_output["AIPW_GAM_MAXW0"] <- NA_real_
      
      benchmark_output["AIPW_GAM_SUCCESS"] <- 0
    }
  ...
  if (!is.null(fold_T1_lm) &&
    ...
    if (!is.null(aipw_lm_inf)) {
      ...
      estimates["AIPW_LM"] <- NA_real_
      benchmark_output["AIPW_LM_SE_ANA"] <- NA_real_
      benchmark_output["AIPW_LM_COV_ANA"] <- NA_real_
      benchmark_output["AIPW_LM_ESS1"] <- NA_real_
      benchmark_output["AIPW_LM_ESS0"] <- NA_real_
      benchmark_output["AIPW_LM_MAXW1"] <- NA_real_
      benchmark_output["AIPW_LM_MAXW0"] <- NA_real_
      benchmark_output["AIPW_LM_SUCCESS"] <- 0
    }
  }
  
  ##############################################################################
  # 5. GEC can only continue if cross-fitting succeeded
  ##############################################################################
  ...
  if (!is.null(fold_T1_gam) &&
    ...
    ###########################################################################
    # Reconstruct prediction datasets
    ###########################################################################
    ...
    ###########################################################################
    # Propensity model used in GEC sandwich
    ###########################################################################
    ...
    ###########################################################################
    # Check against pi.hat stored by generator
    ###########################################################################
    ...
    ###########################################################################
    # 6. ALL FIVE GEC ENTROPIES REQUIRED BY A2
    ###########################################################################
    ...
    for (ent in c(
      ...
      fit_gec <- tryCatch(
        ...
        error = function(e) {
          ...
          message(
            ent,
            " estimation failed in replication ",
            rep_id,
            ": ",
            conditionMessage(e)
          )
      ...
      inf_gec <- tryCatch(
        ...
        error = function(e) {
          ...
          message(
            ent,
            " inference failed in replication ",
            rep_id,
            ": ",
            conditionMessage(e)
          )
  ...
  ##############################################################################
  # 7. A2 design diagnostics
  ##############################################################################
  
  true_pi <-
    if (!is.null(sim$true_pi)) {
      sim$true_pi
    } else {
      rep(
        NA_real_,
  ...
  misc_output <- c(
    ...
    ###########################################################################
    # Target and realized response/treatment rates
    ###########################################################################
...
################################################################################
# A2 MONTE CARLO WRAPPER
#
# Repeats () M times
# for one (N, target_rate) condition
################################################################################

run_A2_stress_condition <- function(
    N,
    target_rate,
    M = 500,
    ps_strength = 2,
  ...
  # fixed calibrated intercept for this target treatment rate
  ps_intercept <-
    as.numeric(
      A2_intercepts[
        as.character(target_rate)
      ]
    )
...
################################################################################
# 1. Convert A2 results to long format
################################################################################
...
make_A2_method_rows <- function(dat, method) {
  
  get_col <- function(name) {
    
    if (name %in% names(dat)) {
      return(as.numeric(dat[[name]]))
    }
  ...
  data.frame(
    ...
    N =
      as.numeric(dat$N_DESIGN),
    
    target_rate =
      as.numeric(dat$RATE_DESIGN),
    
    MC_REP =
      as.integer(dat$MC_REP),
...
make_finite_sample_table <- function(
    results,
    methods = c(
      "IPW",
      "EBPS",
  ...
  for (sc in names(results)) {
    ...
    for (meth in methods) {
      ...
      # ----------------------------------------------------------
      # PURE point-estimator validity:
      # no analytic SE requirement
      # ----------------------------------------------------------
...
###############################################################################
###############################################################################
# A3: HIGH-DIMENSIONAL EXACT-CALIBRATION STRESS TEST
#
# Purpose:
#   Keep the original DGP, propensity model, cross-fitting, entropy maps,
#   Newton/backtracking solver, and ATE definition unchanged.
#
#   Only increase the dimension of the balancing-function matrix b_mat.
#
#   For each q we record:
#       1. ATE estimate
#       2. solver convergence/failure
#       3. exact calibration residual
#       4. condition number of the UNRIDGED dual Hessian
#       5. rank of the dual Hessian
#       6. number of Newton iterations
#
# IMPORTANT:
#   q_balance = number of columns of b_mat.
#
#   solve_lambda_dual() itself then uses:
#
#       S = [1, b_mat, g(pi^{-1})]
#
#   Therefore dual dimension = q_balance + 2.
###############################################################################
###############################################################################
...
###############################################################################
# 1. Construct a nested library of A3 balancing functions
###############################################################################

make_A3_balance_matrix <- function(
    dat,
    q_balance
) {
  ...
  q_balance <- as.integer(q_balance)
  ...
  if (
    ...
    stop(
      "q_balance must be a single integer between 1 and 50."
    )
  }
  
  
  if (!"y.hat" %in% names(dat)) {
    
    stop(
      "dat must contain the cross-fitted outcome prediction y.hat."
    )
  }
  ...
  if (!all(xvars %in% names(dat))) {
    
    stop(
      "A3 currently assumes x1, x2, x3, x4 are present."
    )
  }
  ...
  ###########################################################################
  # Large fixed library of candidate balancing functions
  #
  # IMPORTANT:
  #
  ...
  # Therefore:
  #
  #   B_4  is contained in B_10
  #   B_10 is contained in B_20
  #   ...
  #   B_40 is contained in B_50
  #
  # This makes the A3 comparison genuinely nested.
  ###########################################################################
  
  B_full <- cbind(
    
    #########################################################################
    # Prediction-based balance function from the original estimator
    #########################################################################
    ...
    #########################################################################
    # Quadratic-by-linear interactions
    #########################################################################
  ...
  ###########################################################################
  # We only need the first 50 candidate balancing functions
  ###########################################################################
  ...
  if (ncol(B_full) < 50) {
    ...
    stop(
      "The A3 candidate basis contains fewer than 50 columns."
    )
  }
  
  
  ###########################################################################
  # Standardize each balancing function
  #
  # This is important for interpreting the Hessian condition number.
  #
  # Otherwise a large condition number could simply come from variables
  # measured on very different numerical scales.
  #
  # Exact linear balance is invariant to centering/scaling because the
  # solver already contains an intercept constraint.
  ###########################################################################
...
###############################################################################
# 2. Entropy maps needed ONLY for A3 Hessian diagnostics
#
# We are NOT replacing your original entropy functions.
#
# These reproduce the same derivatives already used by solve_lambda_dual().
###############################################################################

A3_entropy_maps <- function(
    entropy
) {
...
###############################################################################
# 3. Compute A3 numerical diagnostics from a solve_lambda_dual() result
#
# This allows us to leave newton_backtracking_dual() COMPLETELY UNCHANGED.
###############################################################################

compute_A3_dual_diagnostics <- function(
    fit,
    D,
    entropy
) {
  ...
  ###########################################################################
  # Recover final/current lambda
  ###########################################################################
  
  if (
    is.null(fit$lambda) ||
    length(fit$lambda) != r ||
    any(!is.finite(fit$lambda))
  ) {
    
    fail_output$hessian_dimension <-
      r
  ...
  #
  # Same equation used by your existing Newton solver:
  #
  #     sum_i S_i (D_i w_i - 1)
  ###########################################################################
  ...
  ###########################################################################
  # UNRIDGED dual Hessian
  #
  # IMPORTANT:
  #
  # Your Newton solver adds a tiny ridge before inversion.
  #
  # For A3 we want the condition number of the actual dual Hessian,
  # BEFORE that ridge is added.
  ###########################################################################
  ...
  if (
    is.null(singular_values) ||
    length(singular_values) == 0 ||
    any(!is.finite(singular_values))
  ) {
    
    condition_number <- Inf
    log10_condition_number <- Inf
...
###############################################################################
# 4. A3 version of estimate_ATE_dual()
#
# Only substantive difference from your original estimate_ATE_dual():
#
# ORIGINAL:
#
#     b_mat = matrix(dat$y.hat, ncol = 1)
#
# A3:
#
#     b_mat = make_A3_balance_matrix(dat, q_balance)
#
# Everything else remains conceptually the same.
###############################################################################

estimate_ATE_dual_A3 <- function(
    fold_t1,
    fold_t0,
    q_balance,
    entropy = c(
  ...
  ###########################################################################
  # Reconstruct full cross-fitted data
  ###########################################################################
  ...
  ###########################################################################
  # Sort by ID only for clean alignment
  ###########################################################################
  ...
  ###########################################################################
  # A3 q-dimensional balancing-function matrices
  ###########################################################################
  
  B1 <-
    make_A3_balance_matrix(
      dat = dat1,
      q_balance = q_balance
    )
  ...
  ###########################################################################
  # Use your ORIGINAL solve_lambda_dual()
  #
  # No modification to that function.
  ###########################################################################
  ...
  ###########################################################################
  # Numerical diagnostics EVEN IF solver did not converge
  ###########################################################################
  
  diag1 <-
    compute_A3_dual_diagnostics(
      fit = fit1,
      D = D1,
      entropy = entropy
    )
  ...
  ###########################################################################
  # Check whether usable weights exist
  ###########################################################################
  ...
  ###########################################################################
  # Same closed-form ATE as your original estimator
  ###########################################################################
  ...
  # A3 solver success
  #
  # Both treatment-arm calibrations must converge.
  ###########################################################################
...
###############################################################################
# 5. Helper: safely extract a scalar from diagnostic lists
###############################################################################
...
###############################################################################
# 6. Complete A3 Monte Carlo driver
#
# IMPORTANT:
#
# Within each replication:
#   - generate the dataset ONCE
#   - generate cross-fitted predictions ONCE
#   - use exactly the SAME data/predictions for q=4,...,50
#
# This is better than independently regenerating data for each q.
###############################################################################

run_A3_stress_test <- function(
    n = 2000,
    p = 4,
    M = 1000,
    K = 4,
  ...
  if (
    ...
    stop(
      "Every q value must lie between 1 and 50."
    )
  }
  ...
  for (
    ...
    #########################################################################
    # Generate EXACTLY the same original causal DGP
    #
    # OR2PS1 by default
    #########################################################################
    ...
    #
    # Generate ONCE for the entire q-grid.
    #########################################################################
      ...
      for (
        ...
        for (
          ...
          out[[row_counter]] <- data.frame(
              ...
              Max_Balance_Residual1 =
                NA_real_,
              
              Max_Balance_Residual0 =
                NA_real_,
  ...
  ###########################################################################
  # Remove unused list elements if simulation exited unexpectedly
  ###########################################################################
